<template>
  <small-card v-if="!editMode" class="col-md-4 mb-5 card" :style="{width: '300px'}">
    <router-link :to="'/products/' + product.id" class="mr-3">
      <img :src="product.images[0]" class="card-img-top" alt="..." />
    </router-link>
    <div class="card-body">
      <h5 class="card-title">{{ product.title }}</h5>
      <div class="mt-3 mb-2"><span class="p-1" v-for="i in parseInt(product.rating) "><i class="bi bi-star-fill" ></i></span></div>
  ...
  <small-card v-if="editMode" class="editForm w-50">
    <form @submit.prevent="submitChanges">
      <div class="form-group row g-3 align-items-center">
        <div class="col-auto">
          <label for="rating" class="col-form-label">Please rate this product: </label>
        </div>
        <div class="col-auto">
          <input
            type="number"
        ...
        <div class="col-auto">
          <span class="form-text p-1" v-for="i in 5">
      ...
      <div class="list-inline mt-3 mb-2 d-flex justify-content-between">
        <button class="btn btn-success list-inline-item" type="submit">Save</button>
        <button class="btn btn-warning list-inline-item" @click.stop="exitSingleProduct()">
          Cancel
        </button>
      </div>
    </form>
